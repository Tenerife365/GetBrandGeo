# YouTube Shorts, ready to paste

Nine cuts, one per run. Nothing here has been posted or scheduled.

## How to use this

**Upload the `-silent.mp4`.** It has no audio stream at all, by design. Pick a
track from YouTube's own audio library at upload. Audio picked there is licensed
for the platform and is favoured by its distribution, whereas a self-supplied
track that is not cleared gets muted or region blocked.

**The `-scored.mp4` in the same folder is not the feed upload.** It carries a
BrandGEO-composed track and exists for paid placements, site embeds and decks,
where the in-app library does not exist.

**The title is what gets shown, so it carries the argument in one clause.** No
keyword stuffing, no colon-and-brackets construction, no bracketed year, no all
caps. Paste it as written.

**The description is written to be read by a person who paused on the video**,
not to be crawled. It extends the on-screen argument rather than restating it,
and it carries the source links so the figures are checkable, which is the whole
point of using real ones. The primary link sits after the argument; research
links go last.

**Tags go in the tag field, not in the description.** Three to five per cut,
listed under each block. No trailing hashtag run inside the copy, because it
does not serve the reader the description is written for.

**No figure in any description is illustrative.** Where a number appears it was
measured by BrandGEO's own collection pipeline, and the page it was measured on
is linked in the same block.

---

## What was kept from the existing NOTES.md drafts, and what was replaced

Several `youtube/NOTES.md` files already carried a suggested title and
description written by the agent that built the cut. Those were read first.

| Cut | Title | Description |
|---|---|---|
| 1, loss aversion | **replaced** | **replaced** |
| 2, status threat | **trimmed** | **replaced** |
| 3, curiosity gap | **kept verbatim** | rewritten, same sourcing |
| 4, contrarian | **kept verbatim** | kept, two edits |
| 5, concrete proof | **kept verbatim** | **replaced** |
| 6, utility | **kept verbatim** | kept, compressed |
| 7, loss aversion 2 | **replaced** | kept, two edits |
| 8, status threat 2 | **kept verbatim** | **replaced** |
| 9, curiosity gap 2 | **kept verbatim** | kept, two edits |

The reasons, per change:

1. **Cut 1 title, replaced.** `Your buyers are asking AI. You are not in the
   answer.` The second clause states as fact that this viewer is absent from the
   answer, which is a measurement about them that has not been made. A company
   selling measurement should not guess at a viewer's own numbers in an ad. The
   first clause is fine; the sentence does not survive without the second.
2. **Cut 1 description, replaced.** It reused two on-screen lines close to
   verbatim (`no click, no referrer, and no row in your analytics` and `Search at
   least told you your rank. AI answers tell you nothing by default`), so the
   reader who paused on the video got the video again.
3. **Cut 2 title, trimmed not rewritten.** `Your category already has a default
   answer in AI. It is not you.` The first clause is a claim about how these
   products behave and is fine. The second is the same guess about the viewer as
   cut 1. Dropped the second clause and kept the rest exactly as written.
4. **Cut 2 description, replaced.** Same verbatim-reuse problem: `Ask again
   tomorrow and the same name comes back` and `there is no page two of an answer`
   are both on screen.
5. **Cut 3 title, kept verbatim.** It states the setup and withholds the finding,
   which is the mechanism of this driver, and the five-engine claim is correct for
   a 24 July 2026 city run. Nothing to improve.
6. **Cut 3 description, rewritten.** Its sourcing and its figures were right and
   are preserved unchanged. It carried a typo (`means out documenting`) and
   repeated `Same cities. Same engines.` from scene 4. Same argument, same links,
   different sentences.
7. **Cut 4 title, kept verbatim.** It is the whole argument in one clause and it
   names the belief before disagreeing with it.
8. **Cut 4 description, kept with two edits.** Removed `Not penalised. Not
   downranked. Never named.`, which is scene 6 in different words, and removed a
   sentence asserting how engines build answers from entity and citation signals,
   which is a mechanism claim stated with more confidence than any measurement
   here supports. Everything else is that draft's work.
9. **Cut 5 title, kept verbatim.** Exact, no superlative, and it promises only
   what the video delivers, which matters more on a proof hook than anywhere else.
10. **Cut 5 description, replaced.** It asserted `the first fully unanimous
    result we have measured anywhere in this research programme`. That claim came
    off the Chicago page, and it is false: Boston, collected the same day, reports
    three separate 5 of 5 results. A superlative cannot be verified by reading the
    page that asserts it. The Chicago page itself has since been corrected and now
    reads `one of ten cities in this program to produce` a 5 of 5. The rest of the
    draft was good and its structure is preserved.
11. **Cut 6 title, kept verbatim.** Utility hook, and it promises exactly what
    the video hands over.
12. **Cut 6 description, kept and compressed.** It restates the three steps,
    which is normally a repeat but is correct here: a reader running the check is
    holding a phone and will not scrub back for step two. Tightened, and `always
    looks fine` softened to `tends to read fine` because the absolute was doing no
    work.
13. **Cut 7 title, replaced.** `AI made a shortlist in your category this week`
    asserts an event in this viewer's category on this viewer's timeline. The
    replacement makes the same point as a property of the artefact.
14. **Cut 7 description, kept with two edits.** Removed `That is not what a coin
    toss looks like`, which is scene 8, and changed `before anyone contacts you`
    to `before you are contacted`, because `anyone` quantifies over people and
    the compliance scan failed on it. Its Washington DC sourcing is intact.
15. **Cut 8 title, kept verbatim.** `You have not read it` looks like the same
    guess about the viewer as cuts 1 and 2, and it is not: the engines do not send
    their answers to the companies they describe, so it is a situational fact
    rather than a claim about anyone's behaviour.
16. **Cut 8 description, replaced.** `Being named in that answer is one thing.
    What the sentence says is another.` is scene 3 almost word for word.
17. **Cut 9 title, kept verbatim.** States the setup, withholds nothing it cannot
    back, and traces to the Paris run.
18. **Cut 9 description, kept with two edits.** Removed the verbatim reuse of
    scene 9 (`appeared in zero English answers`) and rephrased `Language was the
    only thing we changed`, which is scene 2. Its three research links are intact.

---

## 1. Loss aversion

**Silent master:** `youtube/shorts/20260729-2200-youtube-silent.mp4`
**Hook driver:** #1, loss aversion
**What it argues:** an AI answered a question about your market today, and the
loss it produced leaves no trace in anything you already measure.
...
**TAGS:** `AI search`, `AI visibility`, `marketing analytics`, `ChatGPT`,
...
**Silent master:** `youtube/shorts/20260729-2318-youtube-silent.mp4`
**Hook driver:** #2, status threat
**What it argues:** the answer slot in your category is already occupied, and an
answer is not a ranking you can climb past.
...
**TAGS:** `AI visibility`, `AI search`, `competitive strategy`, `Perplexity`,
`brand strategy`
...
**Silent master:** `youtube/shorts/20260730-0013-youtube-silent.mp4`
**Hook driver:** #3, curiosity gap
**What it argues:** three cities, one collection day, and the thing that decided
whether the engines agreed was the category rather than the city.
...
**TAGS:** `AI visibility`, `AI search`, `local SEO`, `market research`,
`property management`
...
**Silent master:** `youtube/shorts/20260730-0113-youtube-silent.mp4`
**Hook driver:** #4, contrarian
**What it argues:** a Google rank and presence in an AI answer are two
...
**TAGS:** `SEO`, `AI visibility`, `Gemini`, `search marketing`, `AI search`

---

## 5. Concrete proof

**Silent master:** `youtube/shorts/20260730-0216-youtube-silent.mp4`
**Hook driver:** #5, concrete proof
**What it argues:** here is the exact prompt, the exact engines, and exactly
where one company landed in each answer, including the row that flatters it least.
...
**TAGS:** `AI search`, `AI visibility`, `market research`, `Perplexity`,
`local business`
...
**Silent master:** `youtube/shorts/20260730-0313-youtube-silent.mp4`
**Hook driver:** #6, utility
**What it argues:** three steps you can run yourself in about ten seconds, and
the one thing a single check cannot tell you.
...
**TAGS:** `AI search`, `AI visibility`, `small business marketing`, `ChatGPT`,
`how to`

---

## 7. Loss aversion, second pass

**Silent master:** `youtube/shorts/20260730-0413-youtube-silent.mp4`
**Hook driver:** #1, loss aversion, replication with a different execution
**What it argues:** what you lose to an AI answer is not a visit, it is being
considered at all, and it happens upstream of everything you control.
...
**TAGS:** `AI search`, `AI visibility`, `demand generation`, `market research`,
`Claude`

---

## 8. Status threat, second pass

**Silent master:** `youtube/shorts/20260730-0513-youtube-silent.mp4`
**Hook driver:** #2, status threat, replication with a different execution
**What it argues:** being named is one measurement and being described well is
another, and the sentence attached to your name was written without you.
...
**TAGS:** `AI visibility`, `brand reputation`, `AI search`, `Claude`,
`brand strategy`

---

## 9. Curiosity gap, second pass

**Silent master:** `youtube/shorts/20260730-0613-youtube-silent.mp4`
**Hook driver:** #3, curiosity gap, replication with a different execution
**What it argues:** the same question asked in two languages returned a different
set of companies, not the same set reordered.
...
**TAGS:** `AI search`, `AI visibility`, `international marketing`,
`multilingual SEO`, `market research`
