...
| S2 | Build the ruled ladder end to end | bg-backend Opus, bg-verify | day | **IN PROGRESS 2026-07-31 16:5x** (orchestrator chat). Split and running in parallel: `bg-backend` on `planConfig.ts` + `_cost.js` + `_plans.js` + a `db/` migration for `plan_prompt_caps`; `bg-web` on `brandgeo/web/` pricing and JSON-LD. **Dashboard UI and the Stripe price are held for wave 2**, see S16. **Follow-on 2026-08-01, `ceccbcd` on `claude/vigorous-cannon-e51753`: COMMITTED, awaiting batch push** (AUTONOMY §7). The ladder is now mirrored ONCE for the server, in `_plans.js` `PLAN_LIMITS` read through `planLimit()`, replacing five hand-written copies across `seo-crawl.js`, `seo-draft.js` and `social-publish.js`; the last of those still had a `PLAN_RANK` literal with no `radar`, so every rank was off by one since the tier took index 1. An unpriced plan now returns 0 instead of falling through a caller's fallback, and a load-time assertion refuses to leave any `PLAN_ORDER` member unpriced. Pure refactor: 80 entitlement decisions compared against `origin/main`, zero differences. Guarded by `tests/plan_limits_drift.test.js` (16 checks). **Note the near miss:** the first attempt was written against `sprint-ladder-ruling.md` and set `radar`/`essentials` SEO pages to 0, which `a0a02b2` had already ruled the other way; pushing it would have revoked the landing-page audit from both tiers. Read `git log HEAD..origin/main` for INTENT before rebasing anything older than a day |
