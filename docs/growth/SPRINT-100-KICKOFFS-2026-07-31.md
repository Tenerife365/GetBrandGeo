...
| S21 | Admin Revenue page: Usage tab becomes Revenue with Usage / Cost / Revenue tabs | bg-architect spec, then bg-app + bg-backend Opus | day | **IN PROGRESS 2026-08-02** (chat `#S21 · Admin Revenue page`). RESEARCH DONE + Constantin's decisions recorded (`docs/arch/revenue-report-data-contract.md` §8), including a new Pipeline/opportunity section (§9) added on his instruction. UI DONE (COMMITTED, not pushed): `Usage.tsx` -> `Revenue.tsx`, three tabs, `tsc`+`build` pass, live-checked in demo mode (non-admin gate only — admin content needs the function to SIMULATE against). Backend (TEST fixture harness + `revenue-report.js`) running in background via bg-backend/Opus. Needs a Netlify build: ships INSIDE a batch push (rule 9), ideally S16's wave, never alone |
