...
| S3 | Funnel accept path (roadmap C1, C2, C3) | optimizer, bg-copy, bg-app, bg-verify | day | **PUSHED AND LIVE 2026-07-31.** C1, C1a (`ceb3596`), C1b, C1c, **C2 DONE**, **F5/F6 DONE**. **Live audit RUN and verified end to end** (authorized, ~EUR 0.24): widget to result card to unlock to report, report CTA goes to `/signup?domain=`, counters moved (public audits 3→4, unlocked 1→2, email 1→2), and F6 confirmed by a lead row recording `hubspot_error = not_configured` instead of a silent false. **C3: links ROTATED** (`ff7cae3`), six new links on the same prices, URLs in `STRIPE_CHECKOUT_LINKS`, no fallback. **Payment path PROVEN with a real EUR 0.50 charge: `client_reference_id` DOES survive a Payment Link on this account**, acceptance matched, no false `checkout_without_acceptance`. F1 OPEN BY DECISION 2026-07-31. ⚠️ **BLOCKED ON CONSTANTIN, and checkout is DOWN meanwhile**: `STRIPE_CHECKOUT_LINKS` is not reaching the function so the gate fails closed; redeploy first and read the `accept-terms` log, which now names the cause. Old links must NOT be deactivated until that passes. Also owed: confirm nothing else regressed from the test incident (it overwrote BpR's plan; restored to `growth_pro`). See `docs/qa/s3-e2e-payment-test-2026-07-31.md`. **Note: pushed BEFORE the 2026-08-01 batch rule existed; 4 dashboard commits went out individually.** |
...
| S9 | Scoreboard and checkpoint gates | any session | night-safe | **STRUCTURE DONE 2026-08-05 (night council, Day 5, Gate 1 day).** `docs/growth/SPRINT-100-SCOREBOARD.md` created: `test -f` passes, 30 dated rows (2026-08-01 to 2026-08-30) plus a row-zero baseline slot, the gates, the channel EV table, and the capture rule, all per this task's own spec. **Not fully DONE**: every data cell reads `TBD`, including row zero's baseline paying count and MRR, because this environment has no Stripe or Supabase access (checked: no MCP connector for either in the tool list, no network egress to any live domain). Filling it needs a day-side session with those connectors, told to "close the day" for 2026-08-01 through 2026-08-05 at once. This is now the top action for Constantin; see the brief. |
| S10 | Close the A1-S1 revert gate | bg-backend Opus, bg-verify | day | **NARROWED: two thirds already shipped in `3c3f003`. Only A1-S2-tier remains.** See PART D-0. **REGRESSION FOUND 2026-08-03 (night council, re-verify pass): the harness `node brandgeo-dashboard/tests/package_provisioning.test.js` now exits 1**, 62 of 63 checks pass then crashes on `assert.match(hook, /\.select\('plan_grant_until, plan_source'\)/, ...)`. Cause, confirmed by `git show 13bb92d`: `stripe-webhook.js:492` now reads `.select('plan_grant_until, plan_source, stripe_customer_id')` (three columns, added 2026-08-02 for the package-can-name-its-client feature), so the test's exact-string regex no longer matches. `cur?.plan_source` and `cur?.plan_grant_until` are still read from the response at lines 496-497/500, and the new `scripts/check-package-client-binding.js` (9/9 pass) exercises the same branch behaviorally and is clean, so this reads as a stale test assertion, not a functional break — but it is UNCONFIRMED without a day session updating the assertion and re-running. Until fixed, this harness can no longer prove S10's revert-gate items (A1-S1, A1-S6) are still closed. Day action, ahead of any other S10 work. |
...
| S21 | Admin Revenue page: Usage tab becomes Revenue with Usage / Cost / Revenue tabs | bg-architect spec, then bg-app + bg-backend Opus | day | **PUSHED 2026-08-03, state change found by night council day 4 re-verify, not self-reported.** All 10 batch commits (`db804bb..9f6ac7c`) plus a same-day follow-up (`4b67a8d`, splits the combined table into Clients/Research tabs after Constantin live-tested the deployed page) are confirmed ancestors of `origin/main` HEAD (`0a3dc09`): `git merge-base --is-ancestor db804bb HEAD` and the same for `9f6ac7c` and `4b67a8d` all exit 0. The registry board had not been updated to say so. `brandgeo-dashboard/tests/revenue_report.test.js` re-run clean, 97/97, no regression. Netlify build result (did the deploy actually succeed) is UNVERIFIABLE HERE, no network egress from this environment; `4b67a8d`'s own commit message references Constantin already live-testing the deployed page, which is day-side evidence of a prior successful deploy but not this cycle's own check. The 3 policy calls (F3, F4, Radar pipeline valuation, contract §11) are still open, unaffected by the push |
| S22 | Daily Content Pulse: adaptive layer on the publishing runway, every platform posts every weekday | daily pulse chat (Sonnet) generates, Constantin posts | day | **RULED 2026-08-03** by Constantin: daily posts on all platforms, adapted to sprint progress, industry trends and opportunities, consistent for 30 days so the 08-24 measurement reads clean. Kickoff below S21. Protocol lives in `PUBLISHING-PLAN.md` under "The daily pulse layer (S22)". X under-supply closed by ruling: LinkedIn research posts get X-length trims, generated by the pulse session |
| S23 | Directory and launch-platform registrations: Product Hunt, Uneed, Fazier, DevHunt, SaaSHub, AlternativeTo, Indie Hackers, G2, LinkedIn product link, GBP product link | dedicated chat prepares packs + walks through, Constantin submits (accounts and CAPTCHAs are his hands only) | day | **OPENED 2026-08-03** by Constantin, Product Hunt already started by him. Two lanes: static listings (SaaSHub, AlternativeTo, IH, G2, LinkedIn, GBP) go live this week; launch-mechanics platforms (PH, Uneed, Fazier, DevHunt) stage for the plan's Day 10 LAUNCH (Aug 10), assets staged by Day 7 per the 30-day plan. Every link UTM-tagged `utm_source=<directory>&utm_campaign=launch` per `channel-attribution-spec.md`. Real reviews only on G2, never solicited fakes. Copy follows no-AI-tells. Session output: one submission pack per directory in `docs/growth/launch-directories/` |
...
## S22. Daily Content Pulse: the adaptive layer on the runway

**Ruled by Constantin 2026-08-03.** 100 paying subscribers on an MVP in 30
days is brutal market penetration. The brand must land on the market daily,
on every platform, adapted to our progress, current industry trends and
trending opportunities, and the cadence must be consistent for the full 30
days or the measurement is worthless.

**What already exists, do not rebuild it.** `PUBLISHING-PLAN.md` is the
baseline: a four-week runway of finished assets across 8 platforms, every
link UTM tagged, sidecar txt files on disk, a weekly Plausible scoreboard
and a pre-agreed cut rule (zero emails captured in four weeks = platform
dropped). The runway started 2026-08-03 and is never displaced by S22.

**What S22 adds.** The runway is a fixed queue; it cannot react and it
leaves platforms silent on some weekdays, with X under-supplied at 0.27
units per weekday. The pulse layer fills exactly those gaps:

1. **Coverage rule: every platform gets at least one unit every weekday.**
   Where the runway already schedules a post for a platform that day, the
   pulse adds nothing there. No double posting; the pulse fills silence.
2. **Adaptive sources, in priority order:** (a) sprint progress, public
   numbers ONLY after Constantin approves the specific figure in that day's
   checklist; (b) one industry trend scan per day, with the source cited in
   the sidecar for review, never in the post as filler; (c) opportunity
   pieces (a competitor launch, an AI engine announcement, a relevant news
   hook). Never an invented stat; program-wide claims check the superlative
   ledger first.
3. **X fill, ruled now:** the 8 LinkedIn research posts get X-length trims,
   produced by the pulse session, clearing the "decision owed before week
   2" flag in the publishing plan.
4. **Attribution:** every pulse link carries `utm_campaign=pulse` on top of
   the platform's `utm_source`. Runway posts keep their existing tags. The
   08-24 measurement can then separate the layers, and the cut rule applies
   to each platform's combined number.
5. **Rendering:** existing pipelines only (og-card generator, silent video
   masters, script-based). Media files never enter git; text sidecars stage
   in `docs/growth/social/1-Pending/pulse/YYYY-MM-DD/` following the
   folder's README workflow. No new tooling is built for S22.
6. **Voice:** no em dashes, no en dashes, no AI buzzwords, ever.

**Who is responsible, named:** one daily pulse chat (Sonnet 5) GENERATES
and stages everything paste-ready; **Constantin POSTS**, because no agent
holds a platform login and never will. Budget his side at 25 to 35 minutes
a day. The Friday scoreboard pull already in the publishing plan is the
measurement; the daily council reads it at night.

**Credit economy note (rule 9):** S22 costs zero deploys. Generation is
local, docs commits are free, media stays out of git. One pulse session per
day is the cadence, not one per post.

CONTEXT: read `PUBLISHING-PLAN.md` (today's row plus the S22 section),
yesterday's brief in `SPRINT-100-BRIEFS.md`, the scoreboard if it exists,
and `docs/growth/social/README - how this folder works.md`.
ACTION: produce today's pulse units for every platform the runway leaves
silent, plus the day's posting checklist ordered by time of day.
VERIFY: every unit has a sidecar, every link carries utm_source and
utm_campaign=pulse, zero em or en dashes (`grep -P "[\x{2013}\x{2014}]"`
returns nothing), and any progress number is flagged for approval.
RECORD: append one line to the day's section in `SPRINT-100-BRIEFS.md`
naming what was staged; the scoreboard row stays the close-the-day
session's job.
