...
| S21 | Admin Revenue page: Usage tab becomes Revenue with Usage / Cost / Revenue tabs | bg-architect spec, then bg-app + bg-backend Opus | day | **COMMITTED 2026-08-02, awaiting batch push** (chat `#S21 · Admin Revenue page`). All five CQO gate stages DONE: RESEARCH, TEST (97/97 checks), BUILD (backend + UI), SIMULATE (real Stripe/Supabase data, zero mismatched cents), blocking CQO review (`docs/qa/s21-revenue-report-cqo-review-2026-08-02.md`, verdict PASS WITH FINDINGS, independently reproduced the harness and SIMULATE arithmetic). 10 commits, `db804bb..9f6ac7c`, none pushed. The one blocking finding (F1, a false "same as Cost tab" claim) plus 6 more low-risk findings (F2/F7/F9/F10/F11/F12) are fixed with new harness coverage; 3 policy calls (F3 refund-vs-commission clawback, F4 draft/void invoices in gross, Radar €39-vs-€29 pipeline valuation) are surfaced for Constantin, not decided unilaterally — contract §11. Needs a Netlify build: ships INSIDE a batch push (rule 9), ideally S16's wave, never alone |
