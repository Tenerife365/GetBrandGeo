...
| S21 | Admin Revenue page: Usage tab becomes Revenue with Usage / Cost / Revenue tabs | bg-architect spec, then bg-app + bg-backend Opus | day | **IN PROGRESS 2026-08-02** (chat `#S21 · Admin Revenue page`). RESEARCH, TEST, BUILD (backend + UI) and SIMULATE all DONE, all COMMITTED (8 commits, `db804bb..016af46`), none pushed. `revenue-report.js` fixture harness: 93/93 checks, reproduced independently. SIMULATE reconciled real Stripe/Supabase data to zero mismatched cents (`docs/qa/s21-revenue-report-simulate-2026-08-02.md`) and surfaced a real-data attribution edge (a cost-only client with no Stripe customer) that caught two UI bugs before they shipped — see contract §10. Blocking CQO review (bg-verify/Opus) running now, seat C-0b. Needs a Netlify build: ships INSIDE a batch push (rule 9), ideally S16's wave, never alone |
