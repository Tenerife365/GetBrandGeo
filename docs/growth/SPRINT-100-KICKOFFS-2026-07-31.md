...
| S21 | Admin Revenue page: Usage tab becomes Revenue with Usage / Cost / Revenue tabs | bg-architect spec, then bg-app + bg-backend Opus | day | **IN PROGRESS 2026-08-02** (chat `#S21 · Admin Revenue page`). RESEARCH stage DONE: `docs/arch/revenue-report-data-contract.md`. Key finding: `clients.stripe_customer_id` exists but is populated for only 1/38 clients (the E2E test row); real paying clients were onboarded outside the webhook, so per-client attribution needs a Customer-metadata fallback + an explicit "unattributed" bucket, not a migration. Awaiting Constantin's one-line OK on the contract before TEST/BUILD. Needs a Netlify build: ships INSIDE a batch push (rule 9), ideally S16's wave, never alone |
