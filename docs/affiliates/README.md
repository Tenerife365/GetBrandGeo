...
| Endpoints | `affiliate-redirect.js` (`/r/*`), `affiliate-programs-public.js`, `affiliate-apply.js`, `affiliate-conversions.js` (`/api/affiliate/conversions`), `affiliate-portal.js`, `affiliate-admin.js` (since 2026-09-21 also `accounts.list` and `accounts.attach`, which credit a hand-set-up client account to an affiliate and link its Stripe customer so later hand-issued invoices credit them) |
...
| Tests | `tests/affiliate_core.test.js`, `affiliate_flow.test.js`, `affiliate_auth.test.js`, `affiliate_stripe.test.js`, `affiliate_promo_link.test.js`, `tests/helpers/fake_supabase_mem.js`, `affiliate_test_env.js` |
