...
*(Character count: about 430. Fits X with room; trim hashtags to 2 for Threads if needed.)*
...
- **3 signal types:** confirmed via `detectListPosition`, `detectBulletPosition`, `detectSuperlativeRank` in `_analysis.js`.
...
- **Sentence-index rejection plus 5 test cases (P1 to P5):** confirmed in `brandgeo-dashboard/tests/analysis.test.js`, "position units, list rank only, no sentence index (finding 1.2)" block.
