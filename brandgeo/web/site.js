...
(function() {
  ...
  // Rebuilt 2026-07-28 against a new Stripe catalogue: three products, each
  // holding both its monthly and its annual price, every price carrying
  // metadata.plan so stripe-webhook.js can resolve the tier without depending
  // on a hardcoded id map. Generated by scripts/stripe-create-catalogue.js.
  //
  // Growth PRO is now purchasable. Before this it had no Stripe price at all,
  // so its Subscribe button fell through to the signup page and the tier could
  // not be bought.
  //
  // If a URL here is ever blanked, that button falls back to the signup href in
  // index.html rather than dead-ending. Managed and Custom Enterprise stay on
  // #contact (sales-assisted) and have no checkout link by design.
  //
  // The four PREVIOUS payment links are still active in Stripe on purpose, so
  // this file and the live site can be deployed without a gap. Retire them only
  // after this version is live on cPanel.
  var STRIPE_LINKS = {
    essentials: {
